...
def kernel_conversion_vanillacyclegan(config, generator, inkernel, outkernel):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    gen_dict = torch.load(config[generator])
    ...
    in_nii_path = glob(os.path.join(config[inkernel], '*.nii.gz'))
    out_nii = os.path.join("/nfs/masi/krishar1/KernelConversionUnpaired/SPIE_journal_extension/baseline_results", "vanillacycleGAN",config[outkernel])
