...
def register_with_mask_non_harmonized():
    ...
    out_non_harm_exp_toinsp = "/nfs/masi/krishar1/SPIE_2025_InhaleExhaleCT/data_split/final_regsitered_data_with_masks/SyN_exptoinsp_STANDARD_non_harmonized"
    ...
    jd_non_harm_exp_toinsp = "/nfs/masi/krishar1/SPIE_2025_InhaleExhaleCT/data_split/final_regsitered_data_with_masks/jacobian_determinant_exptoinsp_nonharmonized"
    log_jd_non_harm_exp_toinsp = "/nfs/masi/krishar1/SPIE_2025_InhaleExhaleCT/data_split/final_regsitered_data_with_masks/logjacobian_determinant_exptoinsp_nonharmonized"
    ...
    for i in tqdm(range(len(bone_non_harm_files))):
        ...
        out_non_harm_insptoexp_file = os.path.join(out_non_harm_insptoexp, bone_non_harm_files[i])
        ...
        jacobian_determinant_insp_to_exp = ants.create_jacobian_determinant_image(std_non_harm_img, reg_exp_to_insp['invtransforms'][0])
        log_jacobian_determinant_insp_to_exp = ants.create_jacobian_determinant_image(std_non_harm_img, reg_exp_to_insp['invtransforms'][0], do_log=True)
...
def register_with_mask_harmonized():
    ...
    for i in tqdm(range(len(bone_harm_files))):
        ...
        warped_img_insp_to_exp = ants.apply_transforms(fixed = std_non_harm_img, moving = bone_harm_img, transformlist = reg_exp_to_insp['invtransforms'])
        jacobian_determinant_insp_to_exp = ants.create_jacobian_determinant_image(std_non_harm_img, reg_exp_to_insp['invtransforms'][0])
        log_jacobian_determinant_insp_to_exp = ants.create_jacobian_determinant_image(std_non_harm_img, reg_exp_to_insp['invtransforms'][0], do_log=True)
