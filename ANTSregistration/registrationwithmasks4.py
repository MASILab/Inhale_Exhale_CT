...
def register_with_mask_harmonized_original_resolution_insptoexp():
    ...
    out_harm_exp_toinsp = "/nfs/masi/krishar1/SPIE_2025_InhaleExhaleCT/data_split/registration_original_resolution/harmonized/SyN_insptoexp"

    jd_harm_exp_toinsp = "/nfs/masi/krishar1/SPIE_2025_InhaleExhaleCT/data_split/registration_original_resolution/harmonized/jacobian_determinant_insptoexp_harmonized"
    log_jd_harm_exp_toinsp = "/nfs/masi/krishar1/SPIE_2025_InhaleExhaleCT/data_split/registration_original_resolution/harmonized/logjacobian_determinant_insptoexp_harmonized"

    print(out_harm_exp_toinsp)
    print(jd_harm_exp_toinsp)
    print(log_jd_harm_exp_toinsp)
    ...
    for i in tqdm(range(len(bone_harm_files))):
        ...
        out_harm_exp_toinsp_file = os.path.join(out_harm_exp_toinsp, bone_harm_files[i])

        jd_harm_exp_toinsp_file = os.path.join(jd_harm_exp_toinsp, bone_harm_files[i])
        log_jd_harm_exp_toinsp_file = os.path.join(log_jd_harm_exp_toinsp, bone_harm_files[i])
        ...
        reg_exp_to_insp = ants.registration(moving = bone_harm_img, fixed = std_non_harm_img, type_of_transform = 'SyN', moving_mask= bone_harm_mask_img, mask = std_non_harm_mask_img)
        warped_img_exp_to_insp = ants.apply_transforms(moving = bone_harm_img, fixed = std_non_harm_img, transformlist = reg_exp_to_insp['fwdtransforms'])
...
register_with_mask_harmonized_original_resolution_insptoexp()